...
export const chainName = (
  ...
  if (types.length === 1) {
    ...
    if (type.tag === TypeTag.Scalar) {
      return readOrWrite[type.value];
    } else if (isSchemaElement(type)) {
      return `${makeFuncName(lookup(type))}`;
    }
    throw new Error('incomplete chain');
  }

  return makeFuncName(
    types.map(t => nameOfTopLevelTypeOnly(t, lookup)).join('')
  );
};
...
export const schema2tsBlocks = ({
  ...
  const typeSerdesToSort: PieceToSort[] = unique(
    ...
    const sourceTypeAsString = typeToString(fromType, lookup);
